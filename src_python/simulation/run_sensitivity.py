...
def _apply_sample(
    ...
    for name, spec in ordered_items:
        _apply_parameter(out, name=name, spec=spec, value=float(sample[name]))
    return out


def _settings(configs: dict[str, Any]) -> tuple[dict[str, Any], int]:
    registry = configs.get("parameter_distributions", {})
    configured = registry.get("global_sensitivity", {}) if isinstance(registry, dict) else {}
    if configured.get("parameters"):
        return configured, int(registry.get("schema_version", 1))
    # Backward-compatible fallback: legacy min/max specifications are validated
    # and interpreted as uniforms by parameter_distributions.py.
    return configs["sensitivity"], 0
...
def _sample_table(
    ...
    *,
    ...
    draws.insert(0, "sample_design", SAMPLE_DESIGN if schema_version else "latin_hypercube_uniform_legacy")
    draws.insert(0, "uncertainty_schema_version", int(schema_version))
...
def main() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    baseline_country, base_config = _baseline_sensitivity_config(configs)
